...
Shader "Custom/CoolUIShader"
{
    Properties
    {
        ...
        _DistortionSpreader ("Distortion Spreader", float) = 1000
    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            {
                float time = _Time[1] / _SpeedDamper;
                float textureOffsetY = tex2D( _NoiseTex, float2(time, IN.worldPosition.x / _DistortionSpreader + time) ).r;
                float textureOffsetX = tex2D( _NoiseTex, float2(IN.worldPosition.y / _DistortionSpreader + time, time) ).r;

                float2 offset = float2(textureOffsetX, textureOffsetY);
